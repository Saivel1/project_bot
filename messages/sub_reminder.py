...
async def check_subscription_expiry(bot):
    """Проверяет каждую минуту истекающие подписки и отправляет уведомления"""

    while True:
        try:
            current_time = int(datetime.now().timestamp())

            # Диапазон одной минуты
            minute_start = current_time - 30  # -30 секунд
            minute_end = current_time + 30    # +30 секунд

            # Извлекаем пользователей с истекающими подписками
            query = """
            ...
            async with db.pool.acquire() as conn:
                users = await conn.fetch(query, minute_start, minute_end)

            # Отправляем сообщения найденным пользователям
            for user in users:
                user_id = user['user_id']

                text = f"""❌ ПОДПИСКА ИСТЕКЛА

🚫 Доступа к VPN НЕТ до продления подписки
...
⚡ Продлите сейчас и получите мгновенный доступ!"""
